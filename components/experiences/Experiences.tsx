...
const Experiences = ({ experienceData, educationData }: Props) => {
  const [show, setShow] = useState("Experiência");
  ...
  return (
    <SectionWrapper id="experiência" className="min-h-[100dvh] pt-12">
      <h2 className="text-4xl text-center">Experiência</h2>

      <div className="w-fit mx-auto mt-6 p-2 bg-white dark:bg-grey-800 rounded-md flex gap-2 items-center">
        {["Experiência", "Educação"].map((e, i) => (
          <button
            key={i}
            onClick={() => setShow(e)}
      ...
      <div className="lg:container sm:mx-4 lg:mx-auto lg:w-5/6 2xl:w-3/4">
        <div className="relative wrap overflow-hidden p-4 md:py-10 md:px-0">
          ...
          {viewAll
            ? (show === "Experiência" ? experiences : educations).map(
                (e, i) => (
                  // @ts-ignore
                  <ExperienceCard key={i} {...e} index={i} />
                )
              )
            : (show === "Experiência" ? experiences : educations)
                .slice(0, 2)
                .map((e, i) => (
                  // @ts-ignore
                  <ExperienceCard key={i} {...e} index={i} />
      ...
      {(show === "Experiência" ? experiences : educations).length > 2 && (
        <ViewAll
          scrollTo="experience"
